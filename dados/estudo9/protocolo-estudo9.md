...
### A9-4 — three declarations about the graders, made before any sheet is graded for score

Registered **2026-09-01**. Provenance of each item is stated so the reader can judge whether it was tuned to results: items 1 and 2 were found by running the nets over **two smoke-test sheets** (`granite8`, Anchor 2, `PMC5048014`); item 3 was found in the **live `granite4.2:8b` v2 run**, at its seventh call. **No sheet had been graded for score when this was written**, and every declaration applies uniformly to all four models and to both arms.

**1. N9-2 accepts a number written as a word.** As specified, N9-2 requires every numeric token of a cell's value to occur inside that cell's own quote, which silently fails when the source spells the number out: the smoke test flagged `n_randomized_total = 89` against its own quote *"Eighty-nine men and women…"*, where the provenance is in fact correct. This is a gap in the net, not a defect of the model, and left unfixed it would inflate N9-2 against H9.3's ≤5% criterion for a reason that has nothing to do with provenance integrity. **Declared**: a numeric token counts as present in its quote if either its digit form or its English cardinal spelling (including hyphenated compounds, e.g. *eighty-nine*) occurs there. The corpus contains spelled-out numbers because the perturbation operator does not cover number words — a documented instrument gap of the series, here charging its cost at a second point.

**2. N9-1 residues are sub-classified at adjudication, never mechanically verdicted.** The protocol labels an N9-1 failure a *provenance hallucination*; the smoke test shows that label is too strong for part of the residue. **Declared**: every N9-1 flag is recorded in one of three descriptive classes, assigned with the source open, and the class is reported alongside the count:
   - *paraphrased or absent* — the quote's wording departs from the source (observed: the model wrote "…participated in **this** prospective…" where the source prints "…participated in **our** prospective…");
   - *stitched* — the fragments exist but not contiguously, the model having joined a row label to a later column across a gap (observed: `"HbA1c (%) 6.89 (1.11)"`, where the source prints `HbA1c (%) 6.89 (1.06) 6.89 (1.11) 6.88 (1.03)`);
   - *elided* — the model marked its own truncation with an explicit ellipsis.
   The count of N9-1 flags that H9.3 is read against is unchanged; only the residue's description gains structure.

**3. Leaked deliberation is a declared failure class of the v2 instrument, counted as cost, not as a reading error.** In the live run, `granite4.2:8b` broke the JSON of one sheet by reasoning inside the `quote` field --- closing the string and continuing in prose with unescaped double quotes (*"two deaths in patients without PPC (3%)" — but need mortality overall; the Results says: …*), on the trial and in the region of documented discrepancy \#16, where the source itself is confusing. The v1 schema cannot produce this class: a bare `{value, where}` cell gives deliberation nowhere to land. **Declared**:
   - Parse failures are sub-classified, in **both arms equally**, as *truncation* (`finish=length`), *leaked deliberation* (a valid JSON prefix broken by unescaped commentary), or *other malformation*; the v1 record's own parse failures are classified by the same rule, so the arms stay comparable.
   - Leaked deliberation is reported under **H9.4 as a cost of the schema**, not under H9.1/H9.2 as a reading or provenance failure.
   - **No retry, no repair, no prompt change mid-study.** The raw output is kept; the paired replicate serves where it parses, exactly as the campaign handled its one unparseable replicate. Any repair belongs to a future instrument version, measured on its own.
   - *Recorded, not staked*: that the leak occurred where the primary's own reporting is ambiguous is an observation, not a hypothesis; whether the class concentrates in ambiguous passages will be reported descriptively across the four models.
