...
## 11. The orchestrator exchange (Amendment 7) — three models, one frozen harness

Author's directive: keep the reader (gemma12's EXTRA3 sheets) and swap the orchestrator. Identical 10-net harness, identical instruments; every final re-graded mechanically against the EXTRA3 truths.

| Orchestrator | Exact /7 | Where it fails | Pooling |
|---|---|---|---|
| gemma4:12b (base, the reader itself) | 5 | Saslow 2023 (private dispersion route) · Goday (starved at the derivation wall) | consistent, 3 runs |
| codegemma:latest (older-gen code model) | **0** | every CI (wrong arities, ns in bound slots, roles) — MDs copied right, not one interval assembled | not attempted |
| gemma-4-12B-coder (same base, code-tuned) | 5 | Wang (dispersion mess, $[-3.01, 2.37]$) · Goday (starved at the same wall) | **consistent** ($-0.22$ over its own six) |
