...
## Amendment 2 (2026-08-30, registered before the round-2 run): continuous-flow replication, all five extractors

- **Motivation**: round 1 was punctuated by three desktop-app/environment failures on the benchmark machine (a recorded `bash.exe` hang the app died waiting on; an iGPU power-driver kernel event under sustained load). The author ordered a clean replication after updating the app and rebooting the machine.
- **Round-1 status**: the graded iGPU-pair record **stands as published** (its results are valid measurements). The round-1 extension attempt aborted at llama3.1:8b 4/14 extractions when the environment failed; its partial raw files were discarded before any grading — they never entered a result.
- **Round 2**: all five models — `gemma4:12b`, `qwen3:14b`, `llama3.1:8b`, `qwen3.5:9b`, `deepseek-r1:14b` — re-extract **from zero in one uninterrupted queue** ([`e4-rodada2.py`](../../scripts/estudo4/e4-rodada2.py)), outputs under `dados/estudo4/rodada2/`. **Uniform instruments for all five**: the frozen extraction prompt; `num_predict` 4000; the Amendment-1 trigger set (E4-1 neutral sign question; E4-2 all three classes). This also removes round 1's instrument heterogeneity, where the iGPU pair had run under the pre-fix triggers.
- **Memory hygiene** (author's directive): the queue unloads every resident Ollama model before starting and explicitly unloads the previous model between blocks — one model resident at a time.
- **Standing**: round 2 becomes the paper's primary record; round-1 vs round-2 agreement for the iGPU pair is reported as run-to-run stability. Scoring unchanged (§3; graders unchanged; grading of round 2 via `avalia-e4.py` pointed at `rodada2/`).
