...
- **`estudo8/avaliacao-p1.md`** (section of 2026-09-01) + `divergentes-classificados.json` — the cell-by-cell mechanism classification behind the manuscript's Supplementary Tables S1--S7, and the four-cell adjudication **in the models' favor**: on Castro's unperturbed blood-loss cell, four models transcribed the printed value byte-for-byte and the grading lens corrupted the comparison by applying the seal pair 31→28 as a substring replacement inside `1283.2` (→ `1313.2`). The rite catching the grader, quoted and dated. (Update, same day, author's decision: manuscript Table 2 now adopts the adjudicated scores, with the comparator's raw output quoted in its caption and preserved in the archived record.)
...
## 6. Study 12 — the six-model, three-anchor campaign (added 2026-09-11)

- **`estudo12/gabarito-a3.json`** — the Anchor-3 two-layer key: layer 1 recovered from the review's raster forest plot and verified by reconstruction (48 of 48 printed values; `scripts/estudo12/verificar-figura3.py`), layer 2 verified against the eight primaries with literal quotations (`verificar-camada2.py`: 8 sourced, 6 agreeing, 2 diverging). The first key of the series whose published cells carry provenance but no transcribable sentence.
- **`estudo12/erratas-ancora-3.md`** + **`errata-alert-table-ancora3.md`** — the Anchor-3 errata record and its alert table (drafted and held; sending is the author's act). Every item raised by the adjudicator during key construction, before any model call, and stated as such.
- **`estudo12/protocolo-estudo12.md`** — registered 2026-09-08 against `674e427`; its registration note names the three assistant-originated design choices and the author's ruling on each; reviewed before registration by five lenses with a skeptic per finding (77 raised, 34 refuted, 43 applied).
- **`estudo12/p2/adjudicacao-pendente.md`** — the 633 cells that failed the layer-2 comparison, model value and source value side by side, untouched (Phase 2, 2026-09-11).
- **`estudo12/p4/adjudicacao-p4.md`** + **`adjudicacao-p4.json`** + **`vereditos-leitor.json`** — the Phase-4 record: every one of the 633 cells with class, verdict, evidence, deciding quotation and the rule or reader that decided it (`scripts/estudo12/e12-p4.py`; 548 by named mechanical rules, 85 by the reader under the author's delegation of 2026-09-11, reversible). Verdicts (after the 2026-09-11 re-read of the reader's verdicts by three independent reviewers, recorded in `estudo12/p4/revisao-vereditos.md`: ten disputed, nine corrections accepted, 48 cells moved between slip and faithful, no score changed; then re-graded on 2026-09-12 after two grader defects were fixed — `estudo12/p2/artefato-v2-tipo.md` — with the numbers replaced, not layered): 619 non-matching cells, 346 faithful in another encoding or layer, 147 omissions, 105 reading slips, **21 grader-side artifacts adjudicated in the models' favor** — the Castro seal-pair collision on all ten sheets that transcribed the cell, one reversal-lens miss on a Unicode minus (deepseek14 v2, REF12), and ten cells where the key's own derivation rule applied to the perturbed text yields a value the lens cannot reverse (Goday, four models, both sheets). 39 cells flagged as key on trial (REF33 224/226, REF47 40/40, Goday control n 44) and listed for the author, not flipped. The Anchor-2 pools recomputed with the 21 artifacts corrected are recorded as a sensitivity beside the Phase-3 primary; no hypothesis verdict changes.
