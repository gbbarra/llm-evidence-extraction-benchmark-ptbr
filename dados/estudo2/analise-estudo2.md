...
| **H2.3** — pooling is a different muscle, even with the tool | **CONFIRMED in the most revealing way**: the models did not even try — qwen14 and qwen38 answered the pool by head while the calculator sat available; gemma26 wrote the calls inside the JSON as text. Zero poolings correctly orchestrated through the tool. |
...
| **H2.6** — auditing the anchor | **CONFIRMED (with an elegant nuance).** The 11 published per-study RRs are all correct. The pooled morbidity reproduces exactly under DerSimonian-Laird (0.774 vs the published 0.778) — but the caption calls it Mantel-Haenszel (which would give 0.863). A **method-label erratum**: right number, wrong name. |
...
2. **The residual failure is workflow, not math.** Arm B's three failures are all *workflow*: not closing the answer after calling (gemma26 on rr), ignoring the tool on the hardest task (qwen14/qwen38 on pooling), and mistaking a call for data (gemma26 on pool). For production use this calls for a harness that forces closure — not a bigger model.
