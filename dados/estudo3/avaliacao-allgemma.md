...
- Lane S: seven entries whose *study names are fabricated or mangled* ("Tidgren et al.", "Taves", "S.I. et al. (2021)", "Brackgold et al."), pooled **+0.61 [+0.13, +1.09]**.
- *Correction (2026-08-30, found while grading the Amendment-4 arm)*: the fabricated names were **not invented by this arm's arithmetic stage** — they are echoed verbatim from the **Stage-E extraction sheets' `estudo` fields**, where gemma4:12b confabulated author bylines for 5 of 7 trials because the corpus builder strips the article title/byline from the XML (**instrument gap #5**: the model never saw an author name and invented one instead of answering NR). The field was ungraded in the ruler, so the confabulations passed Stage-E grading *and all six audits across three families* unflagged; the baseline calc masked them by using the harness's canonical JSON keys, while the by-head casts surface the sheet's internal field. The fabrication is real and belongs to the cycle — but it was born at extraction, survived every gate, and only became visible here. The moral sharpens: **a local model facing information absent from its input confabulates rather than abstains — at any stage — and only a ruler that grades the field catches it.**
