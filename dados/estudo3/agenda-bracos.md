...
7. **Study 6 pre-registered (2026-08-31) — MA-1 measured same day** ([protocol](../estudo6/protocolo-estudo6.md) · [record](../estudo6/avaliacao-estudo6.md)): both published MA-1 pools **reproduced** from fresh reading (morbidity RR 0.778 ≡ 0.778; mortality 1.023 vs 1.021); zero unexplained residue; zero inventions in 124 cells; stability 96.0%. The replication graded the original in both directions: **anchor erratum #16 confirmed by the author 2026-08-31** (Castro's published "ileus" = the primary's PPC counts; checked against every bowel/GI synonym and the raw untouched PMC XML — genuinely zero mentions, not a corpus artifact) and **two errata of our own key**, still pending sign-off (de Waal ASA contradicting adjudicated erratum #10; Coeckelenbergh blood-loss arms, decided by the abstract's parallel sentence). H6.4 (MA-2 formal lens) measured −0.25 [−0.33, −0.16], fourth lens beside the anchor's −0.24 — Study 6 fully closed; Paper 4 ready to write.
...
Instrument-fix backlog (future corpora/rulers, never retroactive): title/byline kept in corpus text (gap #5 — recurred in Study 4: qwen14 confabulated "Zhou et al. 2019"); `estudo` field graded; perturbation operator covers number words (#1), totals with visible addends (#2), twin tables (#3), rounded prose restatements (#4); neutral trigger question with no embedded premise (E4-1); all three declared trigger classes implemented (E4-2); Wang sign convention scoped to printed-drop fields in `sexteto` (E4-3); one shared bounds/route parser for engine and graders (E4-4); **key corrections from Study 6, never retroactive** — `gabarito-oficial.json` REF29 `asa_*` must follow erratum #10 (GDFT 17:132:95:4) and REF41 `perda_sanguinea_*` must swap arms (GDFT 450 [300–600]; the abstract's parallel sentence fixes the "vs." order).
