...
## Addendum, 2026-09-12 — verdicts re-issued after the grader corrections

Two grader defects (`../p2/artefato-v2-tipo.md`) changed the routed value of several anchor-2 cells without changing what the models had written. Five verdicts were re-issued on the new routed values (V41 0.1 → 0.35 and V42 0.1 → 0.36, an SE the source does not print, now converted; V43 −1.5 → 1.5, V66 −1.15 → 1.15 and V69 −5.65 → 5.65, the sign restored by the parser, the slip unchanged: a bound's sign dropped in the value field, or a between-group interval in the arm's cell) and four new residues decided (Dorans 2022 control SD 0.18 in two sheets — the upper bound −0.02 written where the source prints 0.02; Saslow 2017 control SD 0.74 — bounds of two visits mixed; Dorans 2022 intervention SD 1.13 — a bound's sign dropped in the value field while the type field carries the printed interval; Dorans 2022 intervention SD 0.40 — the net-difference interval as the arm's). All nine are reading slips. Fifteen cells that the first record had charged as omissions or wrong-type dispersions are now correct and left the record; 42 changed value in all. Two earlier verdicts (V38 on 0.8; PMC6024764 exp_n 24) are unused because a mechanical rule now decides those cells the same way.
