...
Na parte 1, os quatro modelos refizeram a extração de dados dos 14 ensaios de uma metanálise publicada com uma célula errada em 624 — mas as sínteses deles, sem ferramenta de agregação, descreviam "tendência favorável" onde a estatística agregada diz "não significativo". O Estudo 2 isola a habilidade que faltou: **a aritmética meta-analítica**. Cada modelo recebe *as próprias extrações* da parte 1 (fidelidade ao que ele mesmo leu) e calcula, por ensaio e agregado: risk ratios, diferenças de médias, intervalos de 95% e os pools de efeitos fixos (Mantel-Haenszel) e aleatórios (DerSimonian-Laird). A correção é 100% mecânica — a verdade de cada quantidade é a recomputação, por funções validadas contra os valores publicados da âncora (o caso-teste as reproduz exatamente: RR 0,573; IC 0,372–0,884), sobre o insumo que o modelo recebeu. Nenhum juiz de linguagem opina.
