...
OVERLAY = {
    ...
    **{(pm, c): dict(veredito="ma-inferiu", valor_fonte="0 (cirurgia aberta; fonte não enumera laparoscopia)",
    ...
    # ================= ESTRATO FECHADO (Emenda 2; adjudicação 2026-08-28) =================
    # Diaper REF26
    ("REF26", "asa_gdft"): dict(veredito="errata-ma", valor_fonte="ASA-PS III-IV: 98 (50.0%)",
                                cit="ASA-PS classes III & IV 98 (50.0) 85 (42.9)",
                                nota="MA publicou 'Not stated'; o artigo reporta (mesmo padrão do Weinberg)"),
    ("REF26", "asa_controle"): dict(veredito="errata-ma", valor_fonte="ASA-PS III-IV: 85 (42.9%)",
                                    cit="ASA-PS classes III & IV 98 (50.0) 85 (42.9)",
                                    nota="MA publicou 'Not stated'"),
    ("REF26", "tempo_flatus_gdft"): dict(veredito="nao-sustentada", valor_fonte="NR",
                                         cit="(a palavra 'flatus' não ocorre no texto integral)",
                                         nota="MA publicou 55±14 h sem sustentação no texto (possível suplemento/figura)"),
    ("REF26", "tempo_flatus_controle"): dict(veredito="nao-sustentada", valor_fonte="NR",
                                             cit="(idem)", nota="MA publicou 58±16 h"),
    # de Waal REF29
    ("REF29", "uso_inotropico"): dict(veredito="pendente-adjudicacao",
                                      nota="MA 'Higher in GDFT'; algoritmo PGDT usa norepinefrina/dobutamina, mas o comparativo de resultados não foi localizado no texto"),
    # Arslan REF30
    ("REF30", "tipo_cirurgia"): dict(veredito="rotulo-razoavel", valor_fonte="cistectomia radical aberta",
                                     cit="complication of intra-abdominal surgeries, including radical cystectomy"),
    ("REF30", "uso_inotropico"): dict(veredito="confirmada-qualitativa",
                                      valor_fonte="comparável entre braços",
                                      cit="Total dose of intraoperative vasopressors (ephedrine or phenylephrine) was comparable between arms",
                                      nota="'No difference' da MA confirmado"),
    ("REF30", "asa_gdft"): dict(veredito="confirmada-nr", valor_fonte="NR (descrição geral ASA 3-4; sem distribuição por braço no texto)",
                                cit="population having an American Society of Anesthesiologists (ASA) score of 3-4"),
    ("REF30", "asa_controle"): dict(veredito="confirmada-nr", valor_fonte="NR", cit="(idem)"),
    # Calvo-Vecino REF33
    ("REF33", "uso_inotropico"): dict(veredito="pendente-adjudicacao",
                                      nota="MA 'Lower in GDFT'; menções só nos métodos"),
    # Coeckelenbergh REF41
    ("REF41", "tipo_cirurgia"): dict(veredito="rotulo-razoavel", valor_fonte="ressecção hepática de grande porte (com provisão laparoscópica)",
                                     cit="when surgeons used a laparoscopic approach (because the software can take this into account…)"),
    ("REF41", "asa_gdft"): dict(veredito="confirmada-nr", valor_fonte="NR", cit="(sem menção a ASA no texto)"),
    ("REF41", "asa_controle"): dict(veredito="confirmada-nr", valor_fonte="NR", cit="(idem)"),
    ("REF41", "fluido_total_gdft"): dict(veredito="nao-sustentada", valor_fonte="NR",
                                         cit="(3500/2800/4500 não ocorrem no texto)",
                                         nota="MA publicou 3500 (2800–4500) sem sustentação no texto"),
    ("REF41", "fluido_total_controle"): dict(veredito="nao-sustentada", valor_fonte="NR",
                                             cit="(3250/2500/4000 não ocorrem no texto)",
                                             nota="MA publicou 3250 (2500–4000)"),
    ("REF41", "tempo_flatus_gdft"): dict(veredito="nao-sustentada", valor_fonte="NR",
                                         cit="(a palavra 'flatus' não ocorre no texto)", nota="MA publicou 52±15 h"),
    ("REF41", "tempo_flatus_controle"): dict(veredito="nao-sustentada", valor_fonte="NR",
                                             cit="(idem)", nota="MA publicou 60±18 h"),
    ("REF41", "uso_inotropico"): dict(veredito="pendente-adjudicacao",
                                      nota="MA 'Lower in GDFT'; desfecho secundário listado, valor não localizado"),
    # Hokenek REF47
    ("REF47", "tipo_cirurgia"): dict(veredito="literal", valor_fonte="histerectomia abdominal total + salpingo-ooforectomia bilateral (TAH+BSO)",
                                     cit="elective total abdominal hysterectomy and bilateral salpingo-oophorectomy (TAH+BSO)"),
    ("REF47", "asa_gdft"): dict(veredito="confirmada-nr", valor_fonte="NR (elegibilidade ASA I-III; sem distribuição)",
                                cit="included 78 patients aged 18-65 years with ASA I-III"),
    ("REF47", "asa_controle"): dict(veredito="confirmada-nr", valor_fonte="NR", cit="(idem)"),
    ("REF47", "uso_inotropico"): dict(veredito="pendente-adjudicacao",
                                      nota="MA 'No difference'; só menção conceitual a inotrópicos"),
    # Lap=0 inferido (cirurgia aberta) — REF26/29/30/47; Coeckelenbergh pendente (provisão lap)
    **{(r, c): dict(veredito="ma-inferiu", valor_fonte="0 (cirurgia aberta; fonte não enumera)",
                    cit=cit, nota="aceitar '0' ou 'NR'")
       for r, cit in [("REF26", "major open abdominal (lap citada só em discussão de literatura)"),
                      ("REF29", "(sem menção a laparoscopia)"),
                      ("REF30", "cistectomia radical aberta"),
                      ("REF47", "TAH+BSO aberta")]
       for c in ("laparoscopia_gdft", "laparoscopia_controle")},
    ("REF41", "laparoscopia_gdft"): dict(veredito="pendente-adjudicacao",
                                         nota="MA=0, mas o texto prevê abordagem laparoscópica; contagem por braço não localizada"),
    ("REF41", "laparoscopia_controle"): dict(veredito="pendente-adjudicacao", nota="idem"),
