...
Run: python scripts/estudo5/pipeline_ma2_e6-gemma.py     (resume-safe at Stage E)
Outputs: dados/estudo5/saidas/EXTRA6M2 · saidas/CALC2 · saidas/POOL6M2 · dados/estudo5/pipeline_ma2_e6/
"""
...
EXTRA6M2 = D5 / "saidas" / "EXTRA6M2"
PIPE6M2 = ROOT / "dados" / "estudo6" / "ma2"
...
def extrai2():
    ...
    (EXTRA6M2 / "verificacoes.json").write_text(json.dumps(verificacoes, ensure_ascii=False, indent=1),
                                              encoding="utf-8")
    print(f"  verificacoes anti-invencao: {sum(1 for v in verificacoes if 'campo' in v)}", flush=True)


def main():
    PIPE6M2.mkdir(exist_ok=True)
    t0 = time.time()

    print("===== MA-2 FORMAL E6 · etapa E: extracao fresca sob schema + rede anti-invencao", flush=True)
    extrai2()

    print("\n===== etapa 1: calculo por estudo (CALC2 = G2b + derivacao declarada)", flush=True)
    base = (D5 / "prompts" / "e5-calc2.txt").read_text(encoding="utf-8")
    for tid in h3.TRIALS:
        if (D5 / "saidas" / "CALC6M2" / f"{tid}.json").exists():
            print(f"  pulando CALC2 {tid} (já existe)", flush=True)
            continue
        e5.roda_estudo("CALC6M2", tid, base, pasta_fichas=EXTRA6M2)

    print("\n===== etapa 2: pooling (instrumentos G3b)", flush=True)
    if (D5 / "resultados-POOL6M2.json").exists():
        print("  pulando POOL6M2 (já existe)", flush=True)
    else:
        e5.roda_g3(origem="CALC6M2", rotulo="POOL6M2")
    pool_reg = json.loads((D5 / "resultados-POOL6M2.json").read_text(encoding="utf-8"))
    proprios = e5.sextetos_do_g2b("CALC6M2")
    ...
    dados = ("AGREGADO (DerSimonian-Laird): MD " + str(ag.get("md")) + " IC95 " + str(ag.get("ic95"))
             ...
             + "\n".join(linhas))
    if (PIPE6M2 / "sintese.md").exists():
        sintese = (PIPE6M2 / "sintese.md").read_text(encoding="utf-8").split("\n\n", 1)[-1].strip()
    ...
    ax.set_title("MA-2 formal (Estudo 6) — 100% gemma4:12b, da extração ao resultado", fontsize=10)
    ax.spines[["left", "top", "right"]].set_visible(False)
    fig.tight_layout()
    fig.savefig(PIPE6M2 / "forest-ma2-e6.png", bbox_inches="tight", facecolor="white")
    ...
    (PIPE6M2 / "correcao-extracao.json").write_text(json.dumps(dict(detalhes=detalhes), ensure_ascii=False,
                                                             indent=1), encoding="utf-8")
    (PIPE6M2 / "resumo.json").write_text(json.dumps(resumo, ensure_ascii=False, indent=1), encoding="utf-8")
    (PIPE6M2 / "sintese.md").write_text("# Síntese do pipeline v2 (100% gemma4:12b)\n\n" + sintese + "\n",
                                      encoding="utf-8")
    ...
    print(f"\n== MA-2 FORMAL E6 COMPLETO em {resumo['minutos']} min · consistente: {consist} · "
