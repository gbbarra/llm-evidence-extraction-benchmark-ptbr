# -*- coding: utf-8 -*-
"""EXTRAI Study 5 — pipeline v3 (Amendment 6, frozen harness): Priority-1 improvements,
...
Run: python scripts/estudo5/pipeline3-gemma.py     (resume-safe at Stage E)
Outputs: dados/estudo5/saidas/EXTRA3 · saidas/CALC2 · saidas/POOL3 · dados/estudo5/pipeline3/
"""
...
def extrai2():
    ...
    (EXTRA3 / "verificacoes.json").write_text(json.dumps(verificacoes, ensure_ascii=False, indent=1),
                                              encoding="utf-8")
    print(f"  verificacoes anti-invencao: {sum(1 for v in verificacoes if 'campo' in v)}", flush=True)


def main():
    PIPE3.mkdir(exist_ok=True)
    t0 = time.time()

    print("===== PIPELINE v3 · etapa E: extracao fresca sob schema + rede anti-invencao", flush=True)
    extrai2()

    print("\n===== etapa 1: calculo por estudo (CALC2 = G2b + derivacao declarada)", flush=True)
    base = (D5 / "prompts" / "e5-calc2.txt").read_text(encoding="utf-8")
    for tid in h3.TRIALS:
        if (D5 / "saidas" / "CALC3" / f"{tid}.json").exists():
            print(f"  pulando CALC2 {tid} (já existe)", flush=True)
            continue
        e5.roda_estudo("CALC3", tid, base, pasta_fichas=EXTRA3)

    print("\n===== etapa 2: pooling (instrumentos G3b)", flush=True)
    if (D5 / "resultados-POOL3.json").exists():
        print("  pulando POOL3 (já existe)", flush=True)
    else:
        e5.roda_g3(origem="CALC3", rotulo="POOL3")
    pool_reg = json.loads((D5 / "resultados-POOL3.json").read_text(encoding="utf-8"))
    proprios = e5.sextetos_do_g2b("CALC3")
    ...
    dados = ("AGREGADO (DerSimonian-Laird): MD " + str(ag.get("md")) + " IC95 " + str(ag.get("ic95"))
             ...
             + "\n".join(linhas))
    if (PIPE3 / "sintese.md").exists():
        sintese = (PIPE3 / "sintese.md").read_text(encoding="utf-8").split("\n\n", 1)[-1].strip()
    ...
    ax.set_title("Pipeline v3 — 100% gemma4:12b, da extração ao resultado", fontsize=10)
    ax.spines[["left", "top", "right"]].set_visible(False)
    fig.tight_layout()
    fig.savefig(PIPE3 / "forest-gemma-v3.png", bbox_inches="tight", facecolor="white")
    ...
    (PIPE3 / "correcao-extracao.json").write_text(json.dumps(dict(detalhes=detalhes), ensure_ascii=False,
                                                             indent=1), encoding="utf-8")
    (PIPE3 / "resumo.json").write_text(json.dumps(resumo, ensure_ascii=False, indent=1), encoding="utf-8")
    (PIPE3 / "sintese.md").write_text("# Síntese do pipeline v3 (100% gemma4:12b)\n\n" + sintese + "\n",
                                      encoding="utf-8")
    ...
    print(f"\n== PIPELINE v3 COMPLETO em {resumo['minutos']} min · consistente: {consist} · "
